...
и Тане пришлось долго её ждать. Поезда всегда ходят точно по расписанию,
и Таня знает, что поезд стоит на платформе ровно одну минуту,
...
Пока Таня стояла на платформе, она насчитала
n поездов на первом пути и
m поездов на втором пути.
Определите минимальное и максимальное время,
которое Таня могла провести на платформе,
...
минимальное и максимальное время в минутах,
которое Таня могла стоять на платформе,
...
func main() {
	...
	var a, b, n, m, totalmin, totalmax int
	fmt.Scan(&a, &b, &n, &m)

	min1 := n + a*(n-1) // Минимальное время которое она находились на платформе, согласно условиям 1 стороны платформы
	max1 := n + a*(n+1) //t Максимальное время которое она находились на платформе, согласно условиям 1 стороны платформы
	min2 := m + b*(b-1) //# Минимальное время которое она находились на платформе, согласно условиям 2 стороны платформы
	max2 := m + b*(b+1) //# Максимальное время которое она находились на платформе, согласно условиям 2 стороны платформы
	...
	if max1 > max2 { //   # Находим общий максимум для обеих сторон плафтормы. Это минимальное из двух значенийt
		totalmax = max2
